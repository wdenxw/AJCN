...
from rl.cuda_parallel_environment import CudaParallelCompressionEnv
from rl.cuda_parallel_ppo_agent import CudaParallelPPOAgent
...
def train_cuda_parallel_rl_compression(config):
    """
    使用CUDA并行训练RL压缩策略
    ...
    """
    # 初始化CUDA并行工具
    cuda_parallel = CudaParallel()
    
    # 设置设备
    device = torch.device("cuda:0" if cuda_parallel.is_available else "cpu")
    print(f"使用设备: {device}")
    ...
    # 创建CUDA并行压缩环境
    print("创建压缩环境...")
    env = CudaParallelCompressionEnv(
        model=model,
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description="CUDA并行RL网络压缩训练")
    ...
    # 运行CUDA并行训练
    train_cuda_parallel_rl_compression(config)
